...
# The one action that can fill a checkpoint panel from empty: index the folder
# the user's own generations are saved to, either by scanning it into this
# library or by giving it a library of its own (routers/libraries.py). Offered
# only when nothing in this library came from a generator at all, so an owner
# whose library is entirely downloaded artwork is never sent looking for a
...
def _checkpoint_empty_action(reason: Optional[str], sd_attributed_images: int) -> Optional[str]:
    """The action worth offering for an empty checkpoint panel, or None.

    Only ``no_checkpoint_metadata`` gets one, and only when no image here came
    from a generator: that is the case where the user's generations are simply
    not indexed yet. If SD output *is* indexed and still records no checkpoint,
    the model name was never written and no amount of scanning will find it, so
    offering a scan would cost a long operation and fail. The other reasons
    already carry their own remedy or honestly have none.
    """
    if reason != REASON_NO_CHECKPOINT_METADATA or sd_attributed_images:
        return None
    return ACTION_SCAN_GENERATED_IMAGES
    ...
    def get_prompt_stats(
        ...
        *,
        ...
        with db.get_db() as conn:
            ...
            # not. This is what separates "your generations are not indexed
        ...
        # An empty panel has to explain itself, because the remedies differ:
        # nothing in this app can invent a checkpoint for an image that was never
        # generated by Stable Diffusion, but the absence of one only describes
        # what has been indexed — the user may well generate images into a folder
        # this library has never seen, which is a real and different situation.
        checkpoint_availability = _checkpoint_availability_reason(
            checkpoints_any, checkpoints_usable
        )
        checkpoint_empty_action = _checkpoint_empty_action(
            checkpoint_availability, sd_attributed_images
        )
        ...
        return {
            ...
            "checkpoint_coverage": {
                ...
                "sd_attributed_images": sd_attributed_images,
                "min_scored_images_per_checkpoint": MIN_SCORED_IMAGES_PER_CHECKPOINT,
            },
            # Shared by all three checkpoint panels, since they share the blocker.
            "checkpoint_empty_action": checkpoint_empty_action,
