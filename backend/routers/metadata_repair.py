...
@router.get(
    ...
    description="""
Counts, per generator, how many **readable** images carry no SD generation
prompt (`missing_prompt`), how many carry no text of any kind — neither a
prompt nor a recovered sidecar caption (`missing_text`), and how many carry a
stored raw metadata envelope that can be replayed without the original file
(`with_raw`). Drives the settings-page "metadata health" row and the
"Recover Missing Text" button beside it.

Every counter, `total` included, describes the same population as the recovery
job: rows pointing at deleted files are excluded, because no run can change one
of them. `scope` names that population and `excluded_unreadable` reports how
many indexed rows were left out. Whole-library composition is what
`GET /api/library-health` reports.
